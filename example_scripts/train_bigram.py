...
if __name__ == "__main__":
    ...
    def bigram_training_loop(
        ...
        if plots:
            ...
            plt.title(
                f"Losses for the {type(model).__name__} model over {max_iters} steps"
            )
    ...
    for model in models:
        model, train_losses, val_losses = bigram_training_loop(
            model, max_iters=max_iters, eval_every=eval_every, plots=True, verbose=True
        )
    ...
    for model in trained_models:
        ...
        chars = decode(
            model.generate(idx=torch.zeros((1, 1), dtype=torch.long), length=1000)[
                0
            ].tolist()
        )
        # Join the characters together and then print the string
