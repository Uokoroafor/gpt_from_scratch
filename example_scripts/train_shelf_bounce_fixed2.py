...
for output_type in ["text", "num"]:
    print(f'Training for output type: {output_type}')
    # output_type = "text"  # 'num' or 'text'
    for use_bpe in [True, False]:
        if use_bpe:
            print("Using BPE")
        else:
            print("Using char-level encoding")
        ...
        encoding_string = "bpe" if use_bpe else "char"

        trainer.log_numerical_outputs(
            test_loader, decode, f"{output_type}_test_log_{encoding_string}.txt", output_type=output_type
        )
